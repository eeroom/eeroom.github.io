...
导出公钥：gpg [-a] -o 公钥导出后的保存的文件  --export key的id  
导出私钥：gpg [-a] -o 私钥导出后的保存的文件  --export-secret-keys key的id  
导出私钥的时候会跳出对话让我们输入私钥的密码！
导出约定：-a参数的意思是指是否使用ascii armor，不论公钥还是私钥，如果指定这个参数，导出的保存文件就用.asc结尾，如果不指定就用.gpg结尾
