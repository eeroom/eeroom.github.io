...
## 复合主键
```
场景：点菜系统（假定其高峰访问量极高），一个主数据库，每个城市部署一台服务器包含对应的sqlite，一台服务器供该地域下多个商家使用，顾客每次在商家点单的时候生成一个订单号，订单号下面关联所有的菜
主数据库：消费表（订单号），自增
         消费明细表（订单号，菜id）
sqlite：消费明细表（菜id，所属订单，名称，价格，等等），菜id自增
定时把sqlite的消费明细表写入到主数据库的消费明细表，这样订单号和菜id都是有序，查询效率仍然很高
```