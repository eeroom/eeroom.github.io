...
df [-h 美化结果] [-i inode相关的信息]
  查看磁盘分区、容量等信息
du [-s 每个文件 -h 美化结果] 路径
  查看指定路径下的所有文件和文件夹的大小，例如：du -sh /usr/*
ln 源路径 目标路径 [-s 符号链接]
...
cat 文件路径
  查看普通文本文件内容
tail [-100f 行数] 文件路径 [& 自动刷新] 
  查看文件文件末尾的内容，场景：查看最新的日志信息，tail -1000f 文件路径 &
...
gzip
  解压缩
tar [x 解压] [c 压缩] [v 进度] [f 文件路径] [z gz] [j bzip2]  [-C ./a/b  等价于先cd到./a/b]
  打包场景：tar cvf 目标路径(../w2t.tar) 源路径(./config)
    逻辑：目标路径 ← 源路径 ，如果源路径不是. ，那么生成的包会包含源路径的目录结构。
    如果不需包换源路径的目录结构，解决办法如下：
      1、cd到源路径内，再执行打包，源路径的值变为.
      2、使用-C ./config/，命令必须为：tar cvf ../w2t.tar -C /config/ .
  解包场景：tar xvf 压缩文件 .
    逻辑：压缩文件 → . ，特别的：. 可以省略，但是不能为别的
    如果想把解压后的文件放到非当前目录下，解决办法如下：
      1、cd到其他目录，然后执行解压缩包
      2、使用-C ./config/ 命令必须为：tar xvf w2t.tar -C ./config/ .
...
~
  扩展，把相对路径转换成绝对路径
  %~dp0 的解释如下：
  %0 代表当前bat文件的路径，可能是完整路径，也可能是部分路径
    如果cmd当前路径是c盘，bat文件在d盘，我们需要在命令行中输入完整的bat文件路径才能执行到此文件，这个时候%0就是完整路径
  d表示驱动器
  p表示路径
  %~dp0 的值一定是当前bat文件的绝对路径，无论命令行的当前路径是多少
  cd /d %~dp0  就能把命令行当前路径切换到bat文件所在路径
%变量名%
